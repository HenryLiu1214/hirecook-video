...
const BigBeat: React.FC<{ text: string; sub?: string; start?: number; colorScheme?: React.ComponentProps<typeof TypewriterText>["colorScheme"]; size?: number; dark?: boolean; wipe?: "ripple" | "slash" }> = ({ text, sub, start = 14, colorScheme = "plum-to-pink", size = 190, dark = false, wipe }) => {
  ...
  return <AbsoluteFill style={{ fontFamily: fonts.display, opacity: m.opacity, clipPath: cp, transform: m.transform, display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 24, perspective: "1400px" }}>
    <div style={{ fontFamily: fonts.display, transform: `scale(${breathe(frame / 60, 1, 0.006)}) rotate(${rotZIn(frame, start, 28, -8)}deg) rotateX(${rotXSettle(frame, start, 32)}deg)` }}>
    ...
    {sub && <div style={{ fontFamily: fonts.display, opacity: interpolate(frame, [112, 126], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" }), fontSize: 38, color: dark ? colors.dimWhite : colors.hcFgSecondary, fontWeight: 650 }}>{sub}</div>}
  </AbsoluteFill>;
};
...
const Brand: React.FC = () => {
  ...
  return <AbsoluteFill style={{ fontFamily: fonts.display, opacity: m.opacity, clipPath: slashWipe(frame, 0, 24), transform: m.transform, display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 24 }}>
    <div style={{ fontFamily: fonts.display, color: colors.hcFgMuted, fontSize: 38, fontWeight: 650 }}>不再測「你像什麼人格」，而是模擬「你在這個環境下會怎麼工作」</div>
    <div style={{ fontFamily: fonts.display, display: "flex", alignItems: "center", gap: 18, padding: "20px 32px", borderRadius: 999, background: "#FFFFFF", border: `1px solid ${colors.hcHairline}`, boxShadow: "0 1px 0 rgba(8,16,40,0.04)" }}>
      <HcLogoMark size={54} />
...
const Formula: React.FC = () => {
  ...
  return <AbsoluteFill style={{ fontFamily: fonts.display, opacity: m.opacity, transform: m.transform, overflow: "hidden" }}>
    <div style={{ fontFamily: fonts.display, position: "absolute", inset: 0, background: "radial-gradient(circle at 50% 42%, rgba(111,227,245,0.18), transparent 40%), radial-gradient(circle at 50% 70%, rgba(33,81,245,0.20), transparent 45%)" }} />
    <div style={{ fontFamily: fonts.display, position: "absolute", inset: 0, opacity: 0.20, backgroundImage: "radial-gradient(circle, rgba(111,227,245,0.38) 1px, transparent 1px)", backgroundSize: "34px 34px", transform: `scale(${1 + settle * 0.03})` }} />
    ...
    <div style={{ position: "absolute", left: "50%", top: "50%", width: 1680, opacity: formulaOpacity, transform: `translate(-50%, -50%) translateY(${formulaY}px) scale(${formulaScale})`, transformOrigin: "center center", textAlign: "center", whiteSpace: "nowrap", fontFamily: fonts.display, fontSize: 300, fontWeight: 850, lineHeight: 0.92, letterSpacing: "-0.07em" }}>
      {formulaChars.map((char, i) => {
        ...
        return <span key={i} style={{ fontFamily: fonts.display, display: "inline-block", opacity: op, transform: `translateY(${ty}em) scale(${sc})`, color }}>{char === " " ? "\u00A0" : char}</span>;
      })}
    </div>

    <div style={{ fontFamily: fonts.display, position: "absolute", left: 0, right: 0, top: 648, display: "flex", alignItems: "center", justifyContent: "center", gap: 16, opacity: interpolate(frame, [156, 190, 324, 356], [0, 1, 1, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp" }) }}>
      {chips.map((x, i) => {
        ...
        return <div key={x.k} style={{ fontFamily: fonts.display, transform: `translateY(${y}px)`, padding: "12px 18px", borderRadius: 999, background: "rgba(5,7,12,0.54)", border: `1px solid ${x.c}44`, boxShadow: `0 18px 60px ${x.c}14`, display: "flex", alignItems: "center", gap: 9 }}>
...
const PE: React.FC = () => {
  ...
  return <AbsoluteFill style={{ fontFamily: fonts.display, opacity: m.opacity, transform: m.transform, display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 26 }}>
    <div style={{ fontFamily: fonts.display, position: "relative", width: 900, height: 360 }}>
      <div style={{ fontFamily: fonts.display, position: "absolute", left: `calc(50% + ${px}px - 160px)`, top: 20, ...circle("P", "Personality", colors.hcCyan) }}><div style={{ fontSize: 96, fontWeight: 850, fontFamily: fonts.mono }}>P</div><div style={{ fontFamily: fonts.display, fontSize: 24 }}>Personality</div></div>
      <div style={{ fontFamily: fonts.display, position: "absolute", left: `calc(50% + ${ex}px - 160px)`, top: 20, ...circle("E", "Environment", colors.hcBlue) }}><div style={{ fontSize: 96, fontWeight: 850, fontFamily: fonts.mono }}>E</div><div style={{ fontFamily: fonts.display, fontSize: 24 }}>Environment</div></div>
...
const RobotAgent: React.FC<{ size: number; boot?: number; label?: string; bodyColor?: string; compact?: boolean }> = ({ size, boot = 1, label, bodyColor = colors.hcBlueDark, compact = false }) => {
  ...
  return <div style={{ fontFamily: fonts.display, width: size, height: compact ? size * 1.22 : size, position: "relative" }}>
    <div style={{ fontFamily: fonts.display, position: "absolute", left: 0, top: compact ? size * 0.12 : 0, width: size, height: size, borderRadius: "50%", background: `linear-gradient(145deg, #0B1434, ${bodyColor})`, border: `${Math.max(2, size * 0.015)}px solid rgba(111,227,245,0.45)`, boxShadow: `0 0 ${size * 0.16 + boot * size * 0.18}px rgba(111,227,245,${0.18 + boot * 0.25}), 0 ${size * 0.08}px ${size * 0.16}px rgba(33,81,245,0.18)` }} />
    <div style={{ fontFamily: fonts.display, position: "absolute", left: "50%", top: compact ? size * 0.43 : size * 0.49, width: faceW, height: faceH, transform: "translate(-50%, -50%)", borderRadius: size * 0.07, background: "rgba(255,255,255,0.11)", border: "1px solid rgba(255,255,255,0.18)" }}>
      <div style={{ fontFamily: fonts.display, position: "absolute", left: faceW * 0.23, top: faceH * 0.34, width: eyeW, height: eyeH, borderRadius: 999, background: colors.hcCyanBright, boxShadow: "0 0 18px rgba(111,227,245,0.88)" }} />
      <div style={{ fontFamily: fonts.display, position: "absolute", right: faceW * 0.23, top: faceH * 0.34, width: eyeW, height: eyeH, borderRadius: 999, background: colors.hcCyanBright, boxShadow: "0 0 18px rgba(111,227,245,0.88)" }} />
      <div style={{ fontFamily: fonts.display, position: "absolute", left: faceW * 0.27, right: faceW * 0.27, bottom: faceH * 0.24, height: Math.max(4, size * (0.018 + boot * 0.012)), borderRadius: 999, background: colors.hcCyanBright, transform: `scaleX(${0.22 + boot * 0.78})`, transformOrigin: "left center", boxShadow: "0 0 16px rgba(111,227,245,0.62)" }} />
    </div>
    {compact && <div style={{ fontFamily: fonts.display, position: "absolute", left: size * 0.26, right: size * 0.26, top: size * 0.02, height: size * 0.12, borderRadius: `${size * 0.08}px ${size * 0.08}px ${size * 0.03}px ${size * 0.03}px`, background: "#FFFFFF", boxShadow: "0 0 0 2px rgba(8,16,40,0.10)" }} />}
...
const BaselineBreak: React.FC = () => {
  ...
  return <AbsoluteFill style={{ fontFamily: fonts.display, opacity: m.opacity, transform: m.transform }}>
    <div style={{ fontFamily: fonts.display, position: "absolute", inset: 0, background: "radial-gradient(circle at 50% 48%, rgba(33,81,245,0.26), transparent 58%), radial-gradient(circle at 50% 58%, rgba(184,58,46,0.18), transparent 46%)" }} />
    <div style={{ fontFamily: fonts.display, position: "absolute", inset: 0, opacity: 0.24, backgroundImage: "radial-gradient(circle, rgba(111,227,245,0.50) 1px, transparent 1px)", backgroundSize: "34px 34px" }} />
    ...
    <div style={{ fontFamily: fonts.display,
      ...
      <div style={{ fontFamily: fonts.display, color: "#FFFFFF", fontSize: 40, fontWeight: 850, letterSpacing: "-0.035em", lineHeight: 1.18 }}>
        <span style={{ fontFamily: fonts.display, color: colors.hcRisk }}>✕</span> 建立基準需要<br />數月內部歷史數據
      </div>
    </div>
    ...
    <div style={{ fontFamily: fonts.display, position: "absolute", left: "50%", top: "47%", width: laserW, height: 5, opacity: formulaOp, transform: "translate(-50%, -50%) rotate(-7deg)", background: "linear-gradient(90deg, transparent, #6FE3F5, #FFFFFF, #6FE3F5, transparent)", boxShadow: "0 0 36px rgba(111,227,245,0.92)" }} />
    ...
    {debris.map((d, i) => {
      ...
      return <div key={i} style={{ fontFamily: fonts.display, position: "absolute", left: "50%", top: "44%", width: 90 * d.s, height: 70 * d.s, opacity: op, transform: `translate(-50%, -50%) translate(${x}px, ${y}px) rotate(${d.r + frame * 0.35}deg)`, borderRadius: "38% 62% 50% 50%", background: "linear-gradient(135deg, #5D6675, #121722)", border: "1px solid rgba(255,255,255,0.08)" }} />;
    })}

    <div style={{ fontFamily: fonts.display, position: "absolute", left: 0, right: 0, bottom: 140, opacity: claimOp, display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 18 }}>
      <div style={{ fontFamily: fonts.display, color: "#FFFFFF", fontSize: 96, fontWeight: 900, letterSpacing: "-0.06em" }}>無需內部歷史數據！</div>
      <div style={{ fontFamily: fonts.display, color: colors.hcCyanBright, fontSize: 40, fontWeight: 760, letterSpacing: "-0.025em" }}>因為我們讓 AI 先幫你玩了。</div>
    </div>
  </AbsoluteFill>;
};
...
const SimulationPipeline: React.FC = () => {
  ...
  return <AbsoluteFill style={{ fontFamily: fonts.display, opacity: m.opacity, transform: m.transform }}>
    <div style={{ fontFamily: fonts.display, position: "absolute", inset: 0, background: "radial-gradient(circle at 50% 50%, rgba(0,180,216,0.20), transparent 48%), radial-gradient(circle at 78% 56%, rgba(33,81,245,0.22), transparent 45%)" }} />
    <div style={{ fontFamily: fonts.display, position: "absolute", inset: 0, opacity: 0.22 + phase2 * 0.26, backgroundImage: "linear-gradient(rgba(111,227,245,0.18) 1px, transparent 1px), linear-gradient(90deg, rgba(111,227,245,0.18) 1px, transparent 1px)", backgroundSize: "46px 46px" }} />
    ...
    <div style={{ fontFamily: fonts.display, opacity: phase1 }}>
      <div style={{ fontFamily: fonts.display, position: "absolute", left: 92, top: 104 }}>
      ...
      <div style={{ fontFamily: fonts.display, position: "absolute", left: 690, top: 210, width: 430, height: 300, borderRadius: 34, background: "linear-gradient(135deg, rgba(33,81,245,0.36), rgba(0,180,216,0.12))", border: "1px solid rgba(111,227,245,0.42)", boxShadow: "0 36px 150px rgba(0,180,216,0.18)", display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 16 }}>
        <div style={{ color: "#FFFFFF", fontSize: 34, fontFamily: fonts.mono, fontWeight: 900 }}>Collab-Overcooked</div>
        <div style={{ fontFamily: fonts.display, color: colors.hcCyanBright, fontSize: 25, fontWeight: 750 }}>game console</div>
        <div style={{ fontFamily: fonts.display, width: 300, height: 18, borderRadius: 999, border: "1px solid rgba(255,255,255,0.20)", background: "rgba(255,255,255,0.06)" }} />
      </div>
      {chips.map((chip, i) => {
        const col = i % 4;
    ...
    <div style={{ fontFamily: fonts.display, opacity: phase2 }}>
      <div style={{ fontFamily: fonts.display, position: "absolute", left: 92, top: 112, color: "#FFFFFF", fontSize: 58, fontWeight: 900, letterSpacing: "-0.05em" }}>High-frequency simulation</div>
      ...
      <div style={{ fontFamily: fonts.display, position: "absolute", left: 520, top: 263, color: colors.dimWhite, fontSize: 32, fontWeight: 720 }}>多壓力情境 × 人格組合模擬</div>
    ...
    <div style={{ fontFamily: fonts.display, opacity: phase3 }}>
      <div style={{ fontFamily: fonts.display, position: "absolute", left: 92, top: 106, color: "#FFFFFF", fontSize: 70, fontWeight: 900, letterSpacing: "-0.055em" }}>Behavioral Fingerprints</div>
      <svg width="1920" height="1080" viewBox="0 0 1920 1080" style={{ fontFamily: fonts.display, position: "absolute", inset: 0, overflow: "visible" }}>
        {curves.map((curve, i) => {
      ...
      {curves.map((curve, i) => {
        ...
        return <div key={curve.label} style={{ fontFamily: fonts.display, position: "absolute", right: 150, top: 392 + i * 98, opacity: op, padding: "16px 24px", borderRadius: 999, background: `${curve.c}1F`, border: `1px solid ${curve.c}77`, color: curve.c, fontSize: 30, fontWeight: 850, boxShadow: `0 20px 90px ${curve.c}24` }}>{curve.label}</div>;
      })}
      <div style={{ fontFamily: fonts.display, position: "absolute", left: 92, bottom: 92, color: colors.dimWhite, fontSize: 30, fontWeight: 650 }}>1,440+ simulations → 可建模的行為證據</div>
    </div>
  </AbsoluteFill>;
};
...
const MBTIInjection: React.FC = () => {
  ...
  return <AbsoluteFill style={{ fontFamily: fonts.display, opacity: enter }}>
    <div style={{ fontFamily: fonts.display, position: "absolute", inset: 0, background: "linear-gradient(180deg, #FFFFFF 0%, #F7F8FB 58%, #EAF0FF 100%)" }} />
    <div style={{ fontFamily: fonts.display, position: "absolute", inset: 0, opacity: 0.20, backgroundImage: "radial-gradient(circle, rgba(33,81,245,0.18) 1px, transparent 1px)", backgroundSize: "32px 32px", transform: `scale(${1 + swirl * 0.04}) rotate(${swirl * 1.2}deg)` }} />
    <div style={{ fontFamily: fonts.display, position: "absolute", left: 0, right: 0, top: titleTop, opacity: titleOp, textAlign: "center", transform: `scale(${titleScale})`, transformOrigin: "center top" }}>
      <div style={{ fontFamily: fonts.display, color: colors.hcFgPrimary, fontSize: 88, fontWeight: 900, letterSpacing: "-0.06em" }}>人格注入</div>
      <div style={{ fontFamily: fonts.display, marginTop: 6, color: colors.hcFgSecondary, fontSize: 28, fontWeight: 680 }}>不是錄用結論，是模擬輸入</div>
    </div>

    <svg width="1920" height="1080" viewBox="0 0 1920 1080" style={{ fontFamily: fonts.display, position: "absolute", inset: 0, overflow: "visible", opacity: interpolate(frame, [116, 146, 286, 330], [0, 1, 1, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp" }) }}>
      {[0, 1, 2].map((i) => {
    ...
    <div style={{ fontFamily: fonts.display, position: "absolute", left: 960, top: 590, width: 340, height: 340, opacity: coreOpacity, transform: "translate(-50%, -50%)", transformOrigin: "center center" }}>
      <RobotAgent size={340} boot={eyeMood} bodyColor="#17339C" />
      <div style={{ fontFamily: fonts.display, position: "absolute", left: 62, right: 62, bottom: 52, height: 12, borderRadius: 999, background: "rgba(255,255,255,0.10)", overflow: "hidden" }}>
        <div style={{ fontFamily: fonts.display, width: `${Math.round(boot * 100)}%`, height: "100%", background: `linear-gradient(90deg, ${colors.hcCyanBright}, #FFFFFF)` }} />
      </div>
    </div>
    <div style={{ fontFamily: fonts.display, position: "absolute", left: 0, right: 0, bottom: 102, opacity: onlineText, textAlign: "center" }}>
      <div style={{ fontFamily: fonts.display, color: colors.hcFgPrimary, fontSize: 66, fontWeight: 900, letterSpacing: "-0.045em" }}>AI Agent Online</div>
      <div style={{ fontFamily: fonts.display, marginTop: 10, color: colors.hcFgSecondary, fontSize: 28, fontWeight: 680 }}>人格訊號已載入，準備進入 Work-Sim</div>
    </div>
    <div style={{ fontFamily: fonts.display, position: "absolute", left: 960, top: 590, width: interpolate(exitRing, [0, 1], [340, 3600]), height: interpolate(exitRing, [0, 1], [340, 3600]), opacity: frame >= 348 ? 1 : 0, background: "#17339C", transform: "translate(-50%, -50%)", borderRadius: "50%", border: `${interpolate(exitRing, [0, 1], [7, 920])}px solid rgba(111,227,245,0.42)`, boxShadow: "0 0 120px rgba(111,227,245,0.36)" }} />
    <div style={{ fontFamily: fonts.display, position: "absolute", left: 960, top: 590, width: 340, height: 340, opacity: frame >= 348 ? 1 : 0, transform: "translate(-50%, -50%)" }}>
...
const OvercookedPlay: React.FC = () => {
  ...
  return <AbsoluteFill style={{ fontFamily: fonts.display, overflow: "hidden", background: "#F7F8FB", transform: `translateX(${exitX}px)` }}>
    <div style={{ fontFamily: fonts.display, position: "absolute", inset: 0, background: "radial-gradient(circle at 71% 46%, rgba(111,227,245,0.18), transparent 36%), linear-gradient(112deg, #071D32 0%, #0D1430 39%, #F7F8FB 39.2%, #EEF4FF 100%)" }} />
    <div style={{ fontFamily: fonts.display, position: "absolute", inset: 0, opacity: 0.16, backgroundImage: "radial-gradient(circle, rgba(111,227,245,0.42) 1px, transparent 1px)", backgroundSize: "34px 34px" }} />

    <div style={{ fontFamily: fonts.display, position: "absolute", left: 70, top: 92, width: 580, opacity: Math.min(entryOp, interpolate(frame, [540, 590], [1, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp" })), transform: `translateY(${entryY}px)` }}>
      <div style={{ color: colors.hcCyanBright, fontFamily: fonts.mono, fontSize: 25, fontWeight: 900, letterSpacing: "0.22em" }}>AI AGENT</div>
      <div style={{ fontFamily: fonts.display, marginTop: 18, color: "#FFFFFF", fontSize: 124, fontWeight: 950, letterSpacing: "-0.078em", lineHeight: 0.82, transform: `scale(${playsPunch})`, transformOrigin: "left center", textShadow: `0 0 ${interpolate(frame, [84, 108, 150], [0, 34, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp" })}px rgba(255,255,255,0.42)` }}>PLAYS</div>
      <div style={{ fontFamily: fonts.display, color: colors.hcCyanBright, fontSize: 114, fontWeight: 950, letterSpacing: "-0.078em", lineHeight: 0.82, transform: `scale(${pressurePunch})`, transformOrigin: "left center", textShadow: `0 0 ${interpolate(frame, [136, 160, 204], [0, 40, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp" })}px rgba(111,227,245,0.55)` }}>PRESSURE</div>
      <div style={{ fontFamily: fonts.display, marginTop: 30, color: colors.dimWhite, fontSize: 35, fontWeight: 820, lineHeight: 1.16, letterSpacing: "-0.04em" }}>遊戲行為<br />轉成可建模訊號</div>
      <div style={{ marginTop: 40, display: "flex", alignItems: "center", gap: 14, color: colors.dimWhite, fontFamily: fonts.mono, fontSize: 19, fontWeight: 850, letterSpacing: "0.13em" }}>
        <span style={{ fontFamily: fonts.mono, width: 120, height: 2, background: `linear-gradient(90deg, ${colors.hcCyanBright}, transparent)`, boxShadow: "0 0 22px rgba(111,227,245,0.42)" }} />
        WORK-SIM SIGNALS
      </div>
    </div>

    <div style={{ fontFamily: fonts.display, position: "absolute", right: 94, top: 148, width: 1080, height: 675, borderRadius: 26, overflow: "hidden", background: "#FFFFFF", border: "1px solid rgba(111,227,245,0.24)", boxShadow: "0 34px 100px rgba(5,7,12,0.24)" }}>
      <Video
      ...
      />
      <div style={{ fontFamily: fonts.display, position: "absolute", inset: 0, background: "linear-gradient(90deg, rgba(5,7,12,0.18) 0%, transparent 22%, transparent 76%, rgba(5,7,12,0.10) 100%)" }} />
      <svg width="1080" height="675" viewBox="0 0 1080 675" style={{ fontFamily: fonts.display, position: "absolute", inset: 0, overflow: "visible", pointerEvents: "none" }}>
    ...
    <div style={{ fontFamily: fonts.display, position: "absolute", left: 72, bottom: 124, width: 530, height: 244, opacity: interpolate(frame, [540, 590], [1, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp" }) }}>
      {tags.map((tag) => {
        const i = tags.indexOf(tag);
        const glow = interpolate(frame, [190 + i * 42, 218 + i * 42, 250 + i * 42], [0, 1, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
        return <div key={tag.en} style={{ fontFamily: fonts.display, position: "absolute", left: 0, top: i * 76, width: 452, padding: "9px 0 10px 0", display: "grid", gridTemplateColumns: "20px 1fr", columnGap: 16, alignItems: "center", borderBottom: "1px solid rgba(111,227,245,0.12)", filter: `brightness(${1 + glow * 0.45})` }}>
          <div style={{ fontFamily: fonts.display, width: 10, height: 10, borderRadius: "50%", background: tag.c, boxShadow: `0 0 24px ${tag.c}` }} />
          <div>
            <div style={{ color: tag.c, fontFamily: fonts.mono, fontSize: 25, fontWeight: 900, letterSpacing: "0.035em" }}>{tag.en}</div>
            <div style={{ fontFamily: fonts.display, marginTop: 4, color: "#FFFFFF", fontSize: 25, fontWeight: 800, letterSpacing: "-0.02em" }}>{tag.zh}</div>
          </div>
        </div>;
      })}
      <div style={{ fontFamily: fonts.display, position: "absolute", left: -12, top: scanY, width: 476, height: 2, opacity: interpolate(frame, [188, 210, 382, 406], [0, 1, 1, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp" }), background: `linear-gradient(90deg, transparent, ${colors.hcCyanBright}, transparent)`, boxShadow: "0 0 26px rgba(111,227,245,0.70)" }} />
    </div>

    <svg width="1920" height="1080" viewBox="0 0 1920 1080" style={{ fontFamily: fonts.display, position: "absolute", inset: 0, overflow: "visible", pointerEvents: "none", opacity: interpolate(frame, [430, 456, 540, 580], [0, 1, 1, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp" }) }}>
    ...
    {frame < 60 && (
      <div style={{ fontFamily: fonts.display, position: "absolute", inset: 0, background: "#17339C", opacity: coverOp, zIndex: 10 }}>
        <div style={{ fontFamily: fonts.display, position: "absolute", left: 960, top: 590, width: 340, height: 340, transform: "translate(-50%, -50%)" }}>
          <RobotAgent size={340} boot={1} bodyColor="#17339C" />
        </div>
      </div>
    )}
...
const FingerprintResults: React.FC = () => {
  ...
  return <AbsoluteFill style={{ fontFamily: fonts.display,
    opacity: m.opacity,
    transform: `${m.transform} translateX(${revealX}px)`,
    overflow: "hidden",
  }}>
    <div style={{ fontFamily: fonts.display, position: "absolute", inset: 0, background: "radial-gradient(circle at 74% 50%, rgba(33,81,245,0.34), transparent 44%), radial-gradient(circle at 78% 75%, rgba(184,58,46,0.14), transparent 38%), linear-gradient(135deg, #05070C 0%, #101521 54%, #071D32 100%)" }} />
    <div style={{ fontFamily: fonts.display, position: "absolute", inset: 0, opacity: 0.13, backgroundImage: "radial-gradient(circle, rgba(111,227,245,0.42) 1px, transparent 1px)", backgroundSize: "34px 34px" }} />
    ...
    <svg width="1920" height="1080" viewBox="0 0 1920 1080" style={{ fontFamily: fonts.display, position: "absolute", inset: 0, overflow: "visible", opacity: analysisDim }}>
      {curves.map((curve, i) => {
    ...
    <div style={{ fontFamily: fonts.display, opacity: analysisDim }}>
      {curves.map((item) => {
        ...
        return <div key={item.value} style={{ fontFamily: fonts.display, position: "absolute", left: 74, top: item.y - 50, width: 430, opacity: op, transform: `translateX(${x}px)` }}>
          ...
          <div style={{ fontFamily: fonts.display, marginTop: 9, color: "#FFFFFF", fontSize: 25, fontWeight: 760, letterSpacing: "-0.02em", lineHeight: 1.18 }}>{item.note}</div>
        </div>;
      })}
    </div>
    ...
    <div style={{ fontFamily: fonts.display, position: "absolute", left: 120, right: 120, top: 120, height: 760, opacity: interpolate(frame, [312, 346, 430, 482], [0, 1, 1, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp" }), transform: `scale(${interpolate(frame, [312, 352], [1.34, 1], { easing: Easing.out(Easing.back(1.0)), extrapolateLeft: "clamp", extrapolateRight: "clamp" })}) rotate(-7deg)`, transformOrigin: "center center", display: "flex", alignItems: "center", justifyContent: "center", pointerEvents: "none" }}>
      <div style={{ fontFamily: fonts.display, width: 900, height: 360, borderRadius: 44, border: `10px solid ${colors.hcWatch}`, color: colors.hcWatch, background: "rgba(5,7,12,0.18)", boxShadow: "0 0 110px rgba(251,191,36,0.20), inset 0 0 42px rgba(251,191,36,0.08)", display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column" }}>
        <div style={{ fontFamily: fonts.mono, fontSize: 38, fontWeight: 900, letterSpacing: "0.14em" }}>IEEE ICEIB 2026</div>
        <div style={{ fontFamily: fonts.display, marginTop: 6, fontSize: 100, fontWeight: 950, letterSpacing: "-0.075em", lineHeight: 0.9 }}>BEST PAPER</div>
        <div style={{ fontFamily: fonts.display, fontSize: 92, fontWeight: 950, letterSpacing: "-0.075em", lineHeight: 0.9 }}>AWARD</div>
      </div>
    </div>

    <div style={{ fontFamily: fonts.display, position: "absolute", inset: 0, opacity: modelFocus, background: "radial-gradient(ellipse at center, rgba(5,7,12,0.92) 0%, rgba(5,7,12,0.74) 42%, rgba(5,7,12,0.18) 76%, transparent 100%)" }} />

    <div style={{ fontFamily: fonts.display, position: "absolute", left: 0, right: 0, top: 320, opacity: interpolate(frame, [536, 584], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" }), transform: `translateY(${interpolate(frame, [536, 584], [44, 0], { easing: Easing.out(Easing.cubic), extrapolateLeft: "clamp", extrapolateRight: "clamp" })}px)`, textAlign: "center" }}>
      <div style={{ fontFamily: fonts.display, color: "#FFFFFF", fontSize: 82, fontWeight: 920, letterSpacing: "-0.06em", lineHeight: 0.98, textShadow: "0 28px 110px rgba(0,0,0,0.60)" }}>
        Behavior Data <span style={{ fontFamily: fonts.display, color: colors.hcCyanBright }}>→</span> P × E Fit Model
      </div>
      <div style={{ fontFamily: fonts.display, marginTop: 26, color: colors.dimWhite, fontSize: 34, fontWeight: 760, letterSpacing: "-0.025em" }}>
        AI 先建立行為基準，不用等內部歷史數據
      </div>
    </div>

    <div style={{ fontFamily: fonts.display, position: "absolute", left: "50%", bottom: 2, width: 6, height: interpolate(frame, [612, 676], [0, 120], { extrapolateLeft: "clamp", extrapolateRight: "clamp" }), opacity: interpolate(frame, [612, 642], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" }), transform: "translateX(-50%)", background: `linear-gradient(180deg, ${colors.hcCyanBright}, transparent)`, boxShadow: "0 0 32px rgba(111,227,245,0.72)" }} />
    <div style={{ fontFamily: fonts.display, position: "absolute", left: "50%", top: 660, width: 278, height: 278, opacity: interpolate(frame, [654, 690], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" }), transform: `translate(-50%, -50%) scale(${interpolate(frame, [654, 696], [0.72, 1], { easing: Easing.out(Easing.back(1.0)), extrapolateLeft: "clamp", extrapolateRight: "clamp" })})`, borderRadius: "50%", background: `linear-gradient(145deg, ${colors.hcBlue}, ${colors.hcCyan})`, boxShadow: "0 30px 120px rgba(33,81,245,0.42), 0 0 90px rgba(111,227,245,0.35)", display: "flex", alignItems: "center", justifyContent: "center", color: "#FFFFFF" }}>
...
const ScienceClose: React.FC = () => {
  ...
  return <AbsoluteFill style={{ fontFamily: fonts.display, opacity: exitOp, transform: `scale(${exitScale})`, overflow: "hidden" }}>
    <div style={{ fontFamily: fonts.display, position: "absolute", inset: 0, opacity: bgOp, background: "linear-gradient(180deg, #FFFFFF 0%, #F7F8FB 62%, #EAF0FF 100%)" }} />
    <div style={{ fontFamily: fonts.display, position: "absolute", inset: 0, opacity: bgOp * 0.22, backgroundImage: "radial-gradient(circle, rgba(33,81,245,0.18) 1px, transparent 1px)", backgroundSize: "32px 32px" }} />

    <div style={{ fontFamily: fonts.display, position: "absolute", left: 0, right: 0, top: 320, opacity: oldTextOp, textAlign: "center" }}>
      <div style={{ fontFamily: fonts.display, color: "#FFFFFF", fontSize: 82, fontWeight: 920, letterSpacing: "-0.06em", lineHeight: 0.98, textShadow: "0 28px 110px rgba(0,0,0,0.60)" }}>
        Behavior Data <span style={{ fontFamily: fonts.display, color: colors.hcCyanBright }}>→</span> P × E Fit Model
      </div>
      <div style={{ fontFamily: fonts.display, marginTop: 26, color: colors.dimWhite, fontSize: 34, fontWeight: 760, letterSpacing: "-0.025em" }}>
    ...
    <div style={{ fontFamily: fonts.display, position: "absolute", left: "50%", top: "50%", width: 620, height: 430, transform: `translate(-50%, -50%) translate(${iconX}px, ${iconY}px) scale(${iconScale})`, transformOrigin: "center center" }}>
      <svg width="620" height="430" viewBox="0 0 620 430" style={{ fontFamily: fonts.display, position: "absolute", inset: 0, overflow: "visible", opacity: interpolate(frame, [15, 30], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" }) }}>
        {[0, 1, 2].map((i) => {
      ...
      <div style={{ fontFamily: fonts.display, position: "absolute", left: "50%", top: "50%", width: circleSize, height: circleSize, transform: `translate(-50%, -50%) translateY(${circleY}px)`, borderRadius: "50%", background: `linear-gradient(145deg, ${colors.hcBlue}, ${colors.hcCyan})`, boxShadow, display: "flex", alignItems: "center", justifyContent: "center", color: "#FFFFFF" }}>
        ...
        <div style={{ fontFamily: fonts.display, position: "absolute", left: "50%", top: "50%", transform: `translate(-50%, -50%) translateY(${calcY}px)`, fontSize: 18, fontWeight: 800, letterSpacing: "0.15em", opacity: calcOp, whiteSpace: "nowrap" }}>CALCULATED</div>
      </div>
    </div>

    <div style={{ fontFamily: fonts.display, position: "absolute", left: 600, right: 80, top: 332, opacity: quoteOp, transform: `translateY(${quoteY}px)`, textAlign: "left" }}>
      <div style={{ fontFamily: fonts.display, color: colors.hcFgPrimary, fontSize: 104, fontWeight: 900, letterSpacing: "-0.065em", lineHeight: 0.92 }}>Right Person.<br /><span style={{ fontFamily: fonts.display, color: colors.hcBlue }}>Fit Environment.</span></div>
      <div style={{ fontFamily: fonts.display, marginTop: 26, color: colors.hcFgSecondary, fontSize: 34, fontWeight: 720, letterSpacing: "-0.025em" }}>用行為證據，判斷人與環境是否真的適配</div>
    </div>
  </AbsoluteFill>;
};
...
export const S2_Science: React.FC = () => {
  ...
  return <AbsoluteFill>
    ...
    <AbsoluteFill style={{ fontFamily: fonts.display, clipPath: slashWipe(frame, 0, 36) }}>
